...
params_definitions <- data.frame( 
  ...
  "mnt" = as.character('microbial turnover rate for nitrifiers'),
  "VmaxN"=as.character('Vmax for NH4 to NO3'),
  "Kn"=as.character("half sat constant for NH4 to NO3"),
  "c_fix_rate"=as.character("rate for C fixation per NH4 to NO3, 1 mol of C per 100 mol of N"),
  "NMicrobe_eff"=as.character("how much microbial growth can happen for N microbes"),
  "minNMicrobeC"=as.character("minimum NMicrobe biomass is RA * min decomposer biomass"))
...
CORPSE <- function(SOM,T,theta,params,claymod=1.0,Litter=FALSE,Bulk=FALSE) {
  ...
  IMM_N_max<-params$max_immobilization_rate*365*(SOM$inorganicNH4+SOM$inorganicNO3)/
    (SOM$inorganicNH4+SOM$inorganicNO3+params$max_immobilization_rate)
  ...
  # if (!Litter & !Bulk){
  #   print(i)
  #   if(loc_Nlim=="TRUE"){print("loc_Nlim")}
  #   if(loc_immob=="TRUE"){print("loc_immob")}
  #   if(loc_Clim=="TRUE"){print("loc_Clim")}
  # 
  # }
  ...
  derivs['CO2']<-CO2prod
  pNH4<-SOM$inorganicNH4/(SOM$inorganicNH4+SOM$inorganicNO3)
  # print(pNH4)
  #The first one should be all 1s, but might be NaNs if initial pool makes this divide by 0 
  if(is.nan(pNH4)){pNH4<-1}
  ##Growth comes from both pools proportionately, using internally named variables NH4_internal and NO3_internal
  if(CN_imbalance_term<0){
    NH4_internal <- CN_imbalance_term*pNH4
    NO3_internal<-CN_imbalance_term*(1-pNH4)
  }else{ ##Excess N min only to NH4 pool
    NH4_internal <- CN_imbalance_term
    NO3_internal<-0
  }
  ...
  non_assimilated_N <- vector(mode='numeric', length=nsites)
  for (ctypes in chem_types) {
...
##Nitrification flux: this location means microbes get mineralized N after decomposition microbes 
  ##but gets deposited N before decomposition microbes
  theta[theta<0]<-0.0
  theta[theta>1.0]<-1.0
  if (params$new_resp_units==TRUE) {
    theta_resp_max<-params$substrate_diffusion_exp/(params$gas_diffusion_exp*(1.0+params$substrate_diffusion_exp/params$gas_diffusion_exp))
  ...
  eq_2 <- (1.0-theta)**params$gas_diffusion_exp/aerobic_max
   ...
   derivs['temp']<-eq_1*eq_2 ##Sending out decomp scalars for nitrification
...
derivs['inorganicNH4']<-NH4_internal +turnover_N_min+non_assimilated_N

derivs['inorganicNO3']<- NO3_internal 
